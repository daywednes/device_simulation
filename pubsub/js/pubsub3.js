...
$(document).ready(function () {
  ...
  controllerChannelSwitch003 = document.getElementById(
    "controller-channel-switch003"
  );
  controllerChannelCancel003 = document.getElementById(
    "controller-channel-cancel003"
  );
  controllerChannel003 = document.getElementById("controller-channel003");

  // select the current channel string on focus
  controllerChannel003.onmouseup = function () {
    return false;
  };
  ...
  controllerChannel003.onkeyup = function (e) {
    if (controllerChannel003.value != controllerChannelId003) {
      ...
      if (
        controllerChannel003.value.length == _idlen &&
        _idpat.test(controllerChannel003.value)
      ) {
...
function setupDemo003() {
  ...
  publishChannel003.onmouseup = function () {
    return false;
  };
  ...
  $(pubTopic003).keyup(function (e) {
    if (isValueChar003(e)) {
      if (checkChannelId003(pubTopic003.value)) {
        // updateCurl();
        $("#pub_topic_full003").text(
          sess003.resolve("api:" + pubTopic003.value)
        );
        pubMessageBtn003.disabled = false;
      } else {
...
function onMessage003(args, kwargs, details) {
  ...
  if (sendTime003) {
    ...
    $("#sub_message_details_time003").text(
