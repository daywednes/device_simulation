...
function connect007() {
   ...
   connection007.onopen = function (session, details) {
      ...
      if (details.x_cb_node_id) {
         updateStatusline007("Connected to node <strong>" + details.x_cb_node_id + "</strong> at " + wsuri);
      } else {
   ...
   connection007.onclose = function(reason, details) {
      ...
      if (details.will_retry) {
         updateStatusline007("Trying to reconnect in " + parseInt(details.retry_delay) + " s.");
      } else {
...
$(document).ready(function()
{
   ...
   controllerChannelSwitch007 = document.getElementById('controller-channel-switch007');
   controllerChannelCancel007 = document.getElementById('controller-channel-cancel007');
   controllerChannel007 = document.getElementById('controller-channel007');

   // select the current channel string on focus
   controllerChannel007.onmouseup = function() { return false; };
   ...
   controllerChannel007.onkeyup = function (e) {
      ...
      if (controllerChannel007.value != controllerChannelId007) {
         ...
         if (controllerChannel007.value.length == _idlen && _idpat.test(controllerChannel007.value)) {
...
function setupDemo007() {
   ...
   publishChannel007.onmouseup = function() { return false; };
   ...
   $("#pub_message007").val("Hello, world.");
   ...
   pubMessageBtn007.onclick = function () {
      ...
      sess007.publish("api:" + $("#pub_topic007").val(), [$("#pub_message007").val()], {}, {acknowledge: true, exclude_me: false}).then(
   ...
   $(pubTopic007).keyup(function(e) {
      ...
      if (isValueChar007(e)) {
         if (checkChannelId007(pubTopic007.value)) {
            // updateCurl();
            $("#pub_topic_full007").text(sess007.resolve("api:" + pubTopic007.value));
            pubMessageBtn007.disabled = false;
         } else {
...
function onMessage007(args, kwargs, details) {
   ...
   if (sendTime007) {
      ...
      $("#sub_message_details_time007").text(diff + " ms / " + event.length + " bytes");
      sendTime007 = null;
   } else {
