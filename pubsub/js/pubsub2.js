...
function connect002() {
   ...
   connection002.onopen = function (session, details) {
      ...
      if (details.x_cb_node_id) {
         updateStatusline002("Connected to node <strong>" + details.x_cb_node_id + "</strong> at " + wsuri);
      } else {
   ...
   connection002.onclose = function(reason, details) {
      ...
      if (details.will_retry) {
         updateStatusline002("Trying to reconnect in " + parseInt(details.retry_delay) + " s.");
      } else {
...
$(document).ready(function()
{
   ...
   controllerChannelSwitch002 = document.getElementById('controller-channel-switch002');
   controllerChannelCancel002 = document.getElementById('controller-channel-cancel002');
   controllerChannel002 = document.getElementById('controller-channel002');

   // select the current channel string on focus
   controllerChannel002.onmouseup = function() { return false; };
   ...
   controllerChannel002.onkeyup = function (e) {
      ...
      if (controllerChannel002.value != controllerChannelId002) {
         ...
         if (controllerChannel002.value.length == _idlen && _idpat.test(controllerChannel002.value)) {
...
function setupDemo002() {
   ...
   publishChannel002.onmouseup = function() { return false; };
   ...
   $("#pub_message002").val("Hello, world.");
   ...
   pubMessageBtn002.onclick = function () {
      ...
      sess002.publish("api:" + $("#pub_topic002").val(), [$("#pub_message002").val()], {}, {acknowledge: true, exclude_me: false}).then(
   ...
   $(pubTopic002).keyup(function(e) {
      ...
      if (isValueChar002(e)) {
         if (checkChannelId002(pubTopic002.value)) {
            // updateCurl();
            $("#pub_topic_full002").text(sess002.resolve("api:" + pubTopic002.value));
            pubMessageBtn002.disabled = false;
         } else {
...
function onMessage002(args, kwargs, details) {
   ...
   if (sendTime002) {
      ...
      $("#sub_message_details_time002").text(diff + " ms / " + event.length + " bytes");
      sendTime002 = null;
   } else {
